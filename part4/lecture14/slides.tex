...
\section{10 Minutes to pandas}

\begin{frame}[t]{10 Minutes to pandas}
...
\begin{frame}[t,fragile]{10 Minutes to pandas}
...
\begin{itemize}
	\item 使用index获得行标号
	\item 使用columns获得列名
	\item 使用values获得内部数据结构
	\item 使用describe获得统计信息
	\item 使用T转置
	\item 使用sort\_index按行标号排序（注：非inplace）
	\item 使用sort\_values(列名)按数值排序（同样非inplace)
\end{itemize}
...
\item 使用.列名返回一个序列
\item 使用[x:y]对行切片
\item 使用.loc[x:y, ['a', 'b']]获取分块
\item 使用.loc[x, ['a', 'b']]获取序列（单列）
\item 使用.loc[x, y]获取单个值
\item 使用.at[x, y]同上
\item 使用.iloc与loc类似，除了是按下标
\item 使用.iat与at类似，除了是按下标
	\end{itemize}
...
使用reindex修改行/列，但若有未出现数据，对应行/列会设置为NaN
使用dropna(how='any')删除NaN列，'any'表示任意，'all'表示所有
使用fillna替换NaN值
...
mean表示均值，0按列，1按行
对某一列，使用value\_counts获取频率
pandas.concat([df1, df2])拼接DataFrame
两个DataFrame用merge归并，类似SQL的连接
使用groupby进行分组
...
On DataFrame, plot() is a convenience to plot all of the
columns with labels:
